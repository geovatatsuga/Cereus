...
export function SettingsView() {
  ...
  return (
    <div className="p-4 sm:p-8 pb-20 max-w-6xl mx-auto w-full space-y-8 relative z-0">
      ...
      <section className="bg-white rounded-xl p-6 border border-slate-200/70 flex flex-col md:flex-row gap-8">
        <div className="flex flex-col items-center gap-4 shrink-0">
          <img src="https://i.pravatar.cc/150?img=32" alt="User" className="w-24 h-24 rounded-full shadow-md" />
          <button className="text-teal-700 font-bold text-xs bg-teal-50 hover:bg-teal-100 px-4 py-2 rounded-lg transition-colors">Alterar foto</button>
        </div>
        <div className="flex-1 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <label className="block">
              <span className="block text-xs font-black text-slate-500 mb-1 uppercase">Nome</span>
              <input type="text" defaultValue="Maria Silva" className="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-2.5 text-sm font-medium text-slate-700 outline-none focus:border-teal-400 focus:ring-2 focus:ring-teal-100 transition-all" />
            </label>
            <label className="block">
              <span className="block text-xs font-black text-slate-500 mb-1 uppercase">E-mail</span>
              <input type="email" defaultValue="maria@exemplo.com.br" className="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-2.5 text-sm font-medium text-slate-700 outline-none focus:border-teal-400 focus:ring-2 focus:ring-teal-100 transition-all" />
            </label>
          </div>
          <button className="bg-slate-900 hover:bg-teal-700 text-white px-6 py-2.5 rounded-lg text-sm font-bold transition-all">Salvar alteracoes</button>
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl p-6 border border-slate-200/70">
          ...
          <div className="space-y-3">
            {integrations.map((integration) => (
              <div key={integration.name} className="flex items-center justify-between gap-4 p-4 border border-slate-100 rounded-lg bg-slate-50/60">
                <div className="flex items-center gap-4">
                  <div className={`${integration.color} w-10 h-10 rounded-lg flex items-center justify-center text-white`}>
                    {integration.icon}
                  </div>
                  <div>
                ...
                <button className={`font-bold text-xs px-4 py-2 rounded-lg transition-colors ${
        ...
        <div className="bg-white rounded-xl p-6 border border-slate-200/70">
          ...
          <div className="space-y-3">
            {roles.map((role) => (
              <div key={role.name} className="p-4 border border-slate-100 rounded-lg bg-slate-50/60">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-slate-900 rounded-xl p-6 text-white">
          ...
          <button className="w-full bg-white text-slate-900 font-bold py-3 rounded-lg">Abrir central LGPD</button>
        </div>
        <div className="lg:col-span-2 bg-white rounded-xl p-6 border border-slate-200/70">
          <div className="flex items-center gap-2 mb-5">
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={action} className="flex items-center justify-between rounded-lg bg-slate-50 border border-slate-100 p-4">
